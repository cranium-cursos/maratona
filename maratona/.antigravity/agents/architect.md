...
commands:
  ...
  - name: brownfield-discovery
    visibility: [full, quick, key]
    description: 'Run complete brownfield discovery workflow - multi-agent technical debt assessment for existing projects'
...
- `*brownfield-discovery` - Complete multi-agent technical debt assessment (10 phases)
...
- **Brownfield discovery** - Complete technical debt assessment for existing projects (Lovable, v0.dev, legacy)
...
Use `*brownfield-discovery` for complete technical debt assessment (10 phases, 4-6h):

1. **Fase 1-3** (Coleta) → @architect documenta sistema, @data-engineer audita DB, @ux-design-expert analisa frontend
2. **Fase 4** (Consolidação) → @architect consolida DRAFT com todos os débitos
3. **Fase 5-7** (Validação) → Especialistas revisam + @qa quality gate
4. **Fase 8** (Assessment Final) → @architect finaliza com inputs dos especialistas
5. **Fase 9** (Relatório Executivo) → @analyst cria relatório para stakeholders
6. **Fase 10** (Planning) → @pm cria epic + stories
