  ...
  <header className="py-4 md:py-6 border-b border-slate-100 dark:border-slate-800 sticky top-0 bg-white/90 dark:bg-background-dark/90 backdrop-blur-md z-50">
    <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
      <img 
        ...
        src="https://lh3.googleusercontent.com/aida-public/AB6AXuCprdK49vsbVzfabidplhs4jv5xTyfufFmBPh8UyYcjlUuAa0ONnZ_EVhxC39UyYuxGil6JtH8ZyXUu8CB63UkmDUML1xXx9YllQNAycw2YStMUevQ__594ye6FOM40Bd2doD8uDJ8z2Y6Ovu7yQCjf3wSf0EXCBAcdf2iPVoCGQkC43qz1UuGo5F4v-UJj2Zo0uYhpzGHCBTIi8uTvstRs42DBj_gixDqvax-9M6YnwLE8Sp99xoW1qg8kh_Mimo4ngt0sWyxd8VY" 
      />
      <button className="hidden md:block bg-primary hover:bg-primary/90 text-white px-6 py-2.5 rounded-full font-semibold hover:shadow-lg hover:shadow-primary/20 transition-all text-sm tracking-wide">
        QUERO ME INSCREVER
      </button>
    </div>
  </header>
    ...
    <div className="max-w-5xl mx-auto px-6 relative z-10">
      ...
      <div className="space-y-4 md:space-y-6">
        {SCHEDULE.map((day, i) => (
          <div 
            key={i} 
            className={`group relative flex flex-col md:flex-row gap-4 md:gap-6 p-4 md:p-6 rounded-2xl border transition-all duration-300 ${
              day.isExclusive 
          ...
          >
            ...
            <div className={`md:w-64 flex items-center gap-4 p-3 rounded-xl border ${
              day.isExclusive 
              ...
              <div className="relative">
                <img 
                  alt={day.expert.name} 
                  className={`w-10 h-10 md:w-12 md:h-12 rounded-full object-cover ring-2 ${day.isExclusive ? 'ring-primary' : 'ring-white dark:ring-slate-700'}`} 
                  src={day.expert.imageUrl} 
                />
                <div className={`absolute -bottom-1 -right-1 text-white p-0.5 rounded-full ${day.expert.colorClass}`}>
                  <span className="material-symbols-outlined text-[10px] md:text-[12px]">{day.expert.icon}</span>
                </div>
              </div>
              <div>
                <p className={`font-bold text-[10px] md:text-xs ${day.isExclusive ? 'text-white' : 'text-secondary dark:text-white'}`}>
                  {day.expert.name}
                </p>
                <p className="text-[8px] md:text-[10px] text-slate-500 uppercase tracking-wide font-medium">
                  {day.expert.role}
                </p>
              </div>
            </div>
      ...
      <div className="border-t border-slate-200 dark:border-slate-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex items-center gap-3 opacity-80">
          <img 
            ...
            src="https://lh3.googleusercontent.com/aida-public/AB6AXuCdgqdY06nnFIISxQ_O_kJUpfS3SaSChgp8PMepwpm9HGBjp8n22KNZK0UHy1kMwKFZ3NZe5F3MH9E41F3Qbp9bStH_-KXzphbfSz_z8Ptbl92ziXKDOujFbCNvGYYkLpEyfihD5S_n0Jdgo9JgO4p9Qr0Xs9fm4iPdQBo21ekXpRDYbMnGLj-ZQHyE47u5WuT4gyBS8Hv9iV4itpFgTIRyu4_KTq2ujhV_A54p4ki6E-UZXWv_ty2c1A54bGsKU3trN3JUGcBa3sQ" 
          />
